          ...
          {showFilter && (
            <div className="bg-white rounded-2xl border border-blue-100 p-4 mb-4 flex flex-wrap gap-2">
              {[
                ...
                { key: "elevated", label: `Elevated (${counts.elevated})` },
                { key: "normal",   label: `Normal (${counts.normal})` },
              ].map(({ key, label }) => (
                <button key={key} onClick={() => setFilter(key)}
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all
                    ${filter === key ? "bg-[#003a66] text-white" : "bg-[#f0f6ff] text-slate-600 hover:bg-blue-100"}`}>
                  {label}
                </button>
              ))}
            </div>
                  ...
                  <div key={item.id} className="bg-white rounded-2xl border border-blue-50 p-4 sm:p-5 flex items-center gap-3 sm:gap-4 hover:shadow-md hover:border-blue-200 transition-all group">
                    ...
                    {item.createdAt && (
                      <div className="hidden sm:flex items-center gap-1.5 text-slate-400 text-xs font-semibold shrink-0">
                        <Clock3 size={12} />{new Date(item.createdAt).toLocaleDateString()}
                      </div>
                    )}
